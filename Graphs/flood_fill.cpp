...
class Solution {
private:
    void dfs(vector<vector<int>>& image, int row, int column, int color, int cur_color, int m, int n){
        if(row >= m || row < 0 || column >= n || column < 0 || image[row][column] != cur_color){
            return;
        }
        image[row][column] = color;
        dfs(image, row + 1, column, color, cur_color, m, n);
        dfs(image, row - 1, column, color, cur_color, m, n);
        dfs(image, row, column + 1, color, cur_color, m, n);
        dfs(image, row, column - 1, color, cur_color, m, n);
    }
...
public:
    vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int color) {
        ...
        dfs(image, sr, sc, color, cur_color, m, n);
        return image;
    }
};