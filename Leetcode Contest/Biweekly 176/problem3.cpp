/*
    ...
    This selection is valid because houses i = 0 and i = 1 have different colors, 
    and house i = 3 is non-adjacent to i = 1.
...
public:
    long long rob(vector<int>& nums, vector<int>& colors) {
        ...
        long long skip = 0;
        for (int i = 1; i < n; i++) {
            long long newTake, newSkip;
            if(colors[i] == colors[i - 1]){
                newTake = skip + nums[i];
            }
            else{
                newTake = max(take, skip) + nums[i];
            }
